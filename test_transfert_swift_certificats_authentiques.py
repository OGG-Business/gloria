#!/usr/bin/env python3
"""
Test Transfert SWIFT avec Certificats Authentiques - Test du transfert SWIFT RÉEL avec certificats BCC authentiques
Transfert RÉEL de 777 USD de BCC RDC vers Monese Ltd avec certificats SWIFT officiels
"""
    ...
    for cert_path, description in certificats:
        try:
            if os.path.exists(cert_path):
                ...
                # Lecture du certificat pour vérification
...
def test_backend_avec_certificats():
    ...
    try:
        ...
        # Test du status SWIFT avec certificats authentiques
...
def test_transfert_swift_avec_certificats_authentiques():
    ...
    # Détails du transfert SWIFT RÉEL avec certificats authentiques
    transfer_data = {
        "amount": 777.0,
        "currency": "USD",
    ...
    try:
        ...
        if response.status_code == 200:
            # SUCCÈS - Transfert réussi avec certificats authentiques
            try:
                data = response.json()
            ...
            # Analyse de l'erreur avec certificats authentiques
            try:
                data = response.json()
                error_detail = data.get('detail', '')
                print(f"   📝 Erreur: {error_detail}")
                
                # Analyse du statut du transfert avec certificats authentiques
